...
export function ShopRequestForm({ locations, existingRequest }: ShopRequestFormProps) {
      ...
      <Card className="border-amber-500/30">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-zinc-400 text-sm mb-3">
            Your shop request for <strong className="text-white">{existingRequest.shop_name}</strong> is pending admin approval.
          </p>
          <Badge variant="warning">PENDING</Badge>
          <p className="text-zinc-500 text-xs mt-4">You&apos;ll receive a notification once reviewed.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    else toast.success("Shop request submitted! Pending admin approval.");
    setLoading(false);
  };
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <p className="text-zinc-400 text-sm">Fill in your shop details. An admin will review and approve your request.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label className="mb-1.5 block">Shop Name</Label>
            <Input name="shop_name" placeholder="e.g. Quick Print Center" required minLength={2} />
          </div>
          ...
          <div>
            ...
            <Input name="address" placeholder="Shop address within campus" required minLength={5} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input name="price_bw" type="number" min={0.5} step={0.5} defaultValue={1.5} required />
            </div>
            <div>
              <Label className="mb-1.5 block">Color Price per page (₹)</Label>
              <Input name="price_color" type="number" min={1} step={0.5} defaultValue={5} required />
            </div>
          </div>
