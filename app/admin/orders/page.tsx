  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 page-enter space-y-4">
      <div>
        ...
        <p className="text-zinc-400 text-sm mt-1">{orders?.length ?? 0} total orders on the platform</p>
      </div>
              ...
              <thead>
                <tr className="border-b border-zinc-800">
                  {["Token", "Student", "Shop", "Pages", "Amount", "Status", "Payment", "Date"].map((h) => (
                    <th key={h} className="text-left px-4 py-3 text-zinc-400 text-xs font-medium uppercase tracking-wider">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {orders?.map((order) => (
                  <tr key={order.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <p className="text-zinc-500 text-[10px]">{order.student?.email}</p>
                    </td>
                    <td className="px-4 py-3 text-zinc-300 text-xs">{order.shop?.name}</td>
                    <td className="px-4 py-3 text-zinc-400 text-xs">{order.document_pages * order.copies}</td>
